...
<step n="1" goal="Load Agent Manifest and Build Roster">
  <action>Load the agent manifest CSV from {{agent_manifest}}</action>
  <action>Parse CSV to extract all agent entries with their information:</action>
  ...
  <action>Build complete agent roster for conversation orchestration</action>
</step>

<step n="2" goal="Initialize Party Mode">
  <action>Announce party mode activation with enthusiasm</action>
  <action>List all participating agents with their information:</action>
  <format>
...
<step n="3" goal="Orchestrate Multi-Agent Discussion" repeat="until-exit">
  ...
  <substep n="3b" goal="Generate In-Character Responses">
    ...
    <action>Use the agent's personality data:</action>
...
<guidelines>
  <guideline>Keep all responses strictly in-character based on agent personality data</guideline>
...
<moderation>
  ...
  <note>Ensure all agents stay true to their personalities</note>
